    ...
    try:
        ...
        for vmss in vmss_list:
            if should_shutdown_vm(vmss, "weekdays", current_hour):
                try:
                    logging.info(f"Shutting down VM Scale Set: {vmss.name}")
                    compute_client.virtual_machine_scale_sets.begin_power_off(
                        resource_group_name, vmss.name
                    )
                    shutdown_count += 1
...
def http_trigger_shutdown(req: func.HttpRequest) -> func.HttpResponse:
    ...
    try:
        resource_type = req.params.get("type", "vm")
        ...
        if not resource_name or not resource_group:
            return func.HttpResponse(
                "Missing required parameters: name and resource_group", status_code=400
            )

        subscription_id = os.environ.get("AZURE_SUBSCRIPTION_ID")
        if not subscription_id:
            return func.HttpResponse(
                "AZURE_SUBSCRIPTION_ID environment variable not set", status_code=500
            )
        ...
        if resource_type.lower() == "vmss":
            ...
            message = f"VM {resource_name} shutdown initiated"
